...
def SaveResultsToFile(final_metrics, model_date_time):
    test_params = final_metrics + [batch_size, epochs, max_lr, margin, f'cifar10_{model_date_time}']
    a = np.asarray(test_params)

    # Open file and append experiment results
    with open(f"{current_abs_path}/Results/cifar10-results.csv", "ab") as f:
...
File_name = "Log/TripletNet/Cifar10/" + str(output_dim) + "d/Cifar10_" + model_date_time + ".pth"
